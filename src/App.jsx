...
import {
  Archive, ArrowLeft, ArrowUp, ArrowUpRight, Buildings, CheckCircle, Compass, CreditCard, EnvelopeSimple, FacebookLogo, FileText, Globe,
  InstagramLogo, List, Lock, MapPin, Phone, UserCircle,
...
function Header({ staticNav = false, checkout = false }) {
  ...
  return <header className={`header ${staticNav ? 'header-static' : ''} ${checkout ? 'checkout-header' : ''}`}>
    <nav className="nav shell" aria-label="Main navigation">
      <Logo />
      {checkout && <span className="checkout-label"><Lock size={16} weight="regular" aria-hidden="true" />Secure checkout</span>}
...
function OrderFlow() {
  ...
  return <section className="order-section" id="order">
    <div className="shell order-shell">
      <div className="order-intro"><div className="order-heading"><h2>Order your Tappy.</h2><p>One white NFC card, ready to connect to the profile or destination you choose.</p></div></div>
      <div className="order-workspace">
        <div className="order-main">
          {step === 1 && <form className="order-form" onSubmit={continueToReview} noValidate>
            <div className="checkout-section-title"><span className="checkout-section-index"><MapPin size={17} aria-hidden="true" /></span><div><h3>Shipping information</h3><p>Where should we send your Tappy card?</p></div></div><div className="quantity-line"><div><h3>White Tappy card</h3><p>Matte finish · NFC enabled</p></div><div className="quantity-control" aria-label="Quantity"><button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity"><Minus size={16} /></button><output aria-live="polite">{quantity}</output><button type="button" onClick={() => setQuantity(Math.min(10, quantity + 1))} aria-label="Increase quantity"><Plus size={16} /></button></div></div>
            <div className="form-fields">
               <label className="wide"><span className="field-label"><UserCircle aria-hidden="true"/>Full name</span><input name="name" autoComplete="name" value={details.name} onChange={update} aria-invalid={Boolean(fieldErrors.name)} aria-describedby={fieldErrors.name ? 'name-error' : undefined} required />{fieldErrors.name && <span className="field-error" id="name-error">{fieldErrors.name}</span>}</label>
               <label><span className="field-label"><EnvelopeSimple aria-hidden="true"/>Email address</span><input type="email" name="email" autoComplete="email" value={details.email} onChange={update} aria-invalid={Boolean(fieldErrors.email)} aria-describedby={fieldErrors.email ? 'email-error' : undefined} required />{fieldErrors.email && <span className="field-error" id="email-error">{fieldErrors.email}</span>}</label>
               <label><span className="field-label"><Phone aria-hidden="true"/>Mobile number</span><input type="tel" name="phone" inputMode="tel" autoComplete="tel" placeholder="09XX XXX XXXX" value={details.phone} onChange={update} aria-invalid={Boolean(fieldErrors.phone)} aria-describedby={fieldErrors.phone ? 'phone-error' : undefined} required />{fieldErrors.phone && <span className="field-error" id="phone-error">{fieldErrors.phone}</span>}</label>
               <label className="wide"><span className="field-label"><MapPin aria-hidden="true"/>Delivery address</span><input name="address" autoComplete="street-address" value={details.address} onChange={update} aria-invalid={Boolean(fieldErrors.address)} aria-describedby={fieldErrors.address ? 'address-error' : undefined} required />{fieldErrors.address && <span className="field-error" id="address-error">{fieldErrors.address}</span>}</label>
               <label><span className="field-label"><Buildings aria-hidden="true"/>City or municipality</span><input name="city" autoComplete="address-level2" value={details.city} onChange={update} aria-invalid={Boolean(fieldErrors.city)} aria-describedby={fieldErrors.city ? 'city-error' : undefined} required />{fieldErrors.city && <span className="field-error" id="city-error">{fieldErrors.city}</span>}</label>
               <label><span className="field-label"><MapPin aria-hidden="true"/>Province</span><select name="province" autoComplete="address-level1" value={details.province} onChange={update} aria-invalid={Boolean(fieldErrors.province)} aria-describedby={fieldErrors.province ? 'province-error' : undefined} required><option value="">Select province</option>{DELIVERY_PROVINCES.map(({ name }) => <option value={name} key={name}>{name}</option>)}</select>{fieldErrors.province && <span className="field-error" id="province-error">{fieldErrors.province}</span>}{deliveryRegion && <small className="delivery-region-note">{deliveryRegion} delivery · ₱{shipping}</small>}</label>
               <label className="wide"><span className="field-label"><Archive aria-hidden="true"/>Postal code</span><input name="postal" inputMode="numeric" autoComplete="postal-code" maxLength="4" value={details.postal} onChange={update} aria-invalid={Boolean(fieldErrors.postal)} aria-describedby={fieldErrors.postal ? 'postal-error' : undefined} required />{fieldErrors.postal && <span className="field-error" id="postal-error">{fieldErrors.postal}</span>}</label>
            </div>
            ...
            <button className="button order-next" type="submit"><span>Review order</span><FileText size={18} aria-hidden="true" /></button>
          </form>}
        ...
        <aside className="order-summary" aria-label="Order summary"><header><h3>Order summary</h3><span>White Tappy card</span></header><img src="/assets/tappy-hero.jpg" alt="White Tappy NFC card" width="900" height="506" /><div><span>White Tappy card × {quantity}</span><b>₱{quantity * 199}.00</b></div><div><span>Delivery</span><b>{deliveryRegion ? `₱${shipping}.00` : 'Calculated at next step'}</b></div><div className="summary-total"><span>Total</span><strong>{deliveryRegion ? `₱${total}.00` : `₱${quantity * 199}.00`}</strong></div><footer><CheckCircle size={20}/><span><b>Secure checkout</b><small>Your information is protected.</small></span></footer></aside>
      </div>
    </div>
  </section>
}
      ...
      {showcase.map((entry, index) => (
        <figure className="home-testimonial" key={index}>
          <div className="home-testimonial-stars" aria-label={`${entry.rating} out of 5 stars`}>
            {[1, 2, 3, 4, 5].map((star) => <Star key={star} size={15} weight={entry.rating >= star ? 'fill' : 'light'} aria-hidden="true" />)}
          </div>
          {entry.comment && <blockquote>{entry.comment}</blockquote>}
          <figcaption><b>{entry.display_name}</b><span>Verified customer</span></figcaption>
        </figure>
      ))}
    </div>
...
export default function App() {
  ...
  const feedbackPage = currentPath === '/feedback'
  ...
  const unknownPage = !['', '/order', '/r', '/faqs', '/privacy', '/terms', '/feedback'].includes(currentPath) && !managedPage && !customerEditor
  useLayoutEffect(() => {
  ...
  useGSAP(() => {
    ...
    return () => media.revert()
  }, { scope: appRef, dependencies: [orderPage, adminPage, howItWorksPage, privacyPage, termsPage, feedbackPage, managedPage, customerEditor, unknownPage] })
  useEffect(() => {
    ...
    document.title = unknownPage ? 'Page not found | Tappy' : adminPage ? 'Tappy Admin' : customerEditor ? 'Edit your Tappy Page' : managedPage ? 'Tappy Page' : orderPage ? 'Order Tappy | Tappy' : feedbackPage ? 'Feedback | Tappy' : publicMeta?.[0] || 'Tappy NFC Card Philippines | One Tap, Every Connection'
    let robots = document.querySelector('meta[name="robots"]')
    if (!robots) { robots = document.createElement('meta'); robots.name = 'robots'; document.head.appendChild(robots) }
    robots.content = unknownPage || adminPage || managedPage || customerEditor || orderPage || feedbackPage ? 'noindex,nofollow,noarchive' : 'index,follow,max-image-preview:large'
  ...
  if (feedbackPage) return <main className="page" ref={appRef}><Header staticNav /><React.Suspense fallback={<RouteLoading/>}><div className="site-curtain"><FeedbackPage/></div></React.Suspense><div className="footer-reveal"><div className="footer-stack"><Footer /></div></div></main>
