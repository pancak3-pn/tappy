...
import { ArrowLeft, ArrowsClockwise, BellRinging, CalendarBlank, ChartLineUp, ChatCircleText, CheckCircle, Clock, Copy, CreditCard, Database, EnvelopeSimple, EnvelopeSimple as Inbox, Eye, GearSix, House, IdentificationCard, Info, Kanban, LinkSimple, ListBullets, LockKey, MagnifyingGlass, MapPin, Package, Palette, PaperPlaneTilt, Plus, Printer, ShieldCheck, ShoppingBag, SignOut, Star, UserCircle, Wallet, WarningCircle, X } from '@phosphor-icons/react'
...
export default function AdminDashboard() {
  ...
  const visibleThreads = messageThreads.filter((thread) => {
    ...
    if (messageFolder === 'support' && thread.order_id) return false
    if (messageFolder === 'all' && !thread.order_id) return false
  ...
  useEffect(() => {
    ...
    setMessageDraft((current) => ({
      ...
      recipientEmail: selectedThread.customer_email,
      subject: selectedThread.order_id ? (/^re:\s*/i.test(selectedThread.subject) ? selectedThread.subject : `Re: ${selectedThread.subject}`) : `Tappy Support — ${selectedThread.subject.replace(/^re:\s*/i, '')}`,
      body: '',
    }))
  ...
  async function loadSystemHealth() {
    try { setSystemHealth(await requestJson('/api/admin/system-health')) }
    catch (requestError) { setSystemHealth({ overall:'down', checkedAt:new Date().toISOString(), checks:{ api:{ status:'unknown', label:'API' } }, activity:{ failedEmails:0, inboundWebhooks:0 }, error:requestError.message }) }
  }
  ...
  async function loadMessages(quiet = false) {
    ...
    } catch (requestError) { if (!quiet) notify(requestError.message, 'error') }
  ...
  useEffect(() => {
    if (!token || adminView !== 'health') return undefined
    loadSystemHealth()
  ...
  return <main className="admin-page">
    ...
    <div className={`admin-shell ${adminView === 'messages' ? 'admin-messages-shell' : ''}`}>
      ...
      {adminView === 'health' && <section className="admin-health-page" aria-labelledby="system-health-title"><header className="admin-health-head"><div><span className="admin-eyebrow">Operations</span><h2 id="system-health-title">System Health</h2><p>Live status for the services that keep Tappy running.</p></div><button type="button" onClick={loadSystemHealth} disabled={!systemHealth}><ArrowsClockwise size={17}/>Refresh</button></header>{systemHealth ? <><div className={`health-overall health-${systemHealth.overall}`}><i aria-hidden="true">{systemHealth.overall === 'operational' ? <CheckCircle size={25}/> : <WarningCircle size={25}/>}</i><div><strong>{systemHealth.overall === 'operational' ? 'All systems operational' : systemHealth.overall === 'degraded' ? 'Some services need attention' : 'Service interruption detected'}</strong><span>Last checked {date(systemHealth.checkedAt)}</span></div></div><div className="health-check-grid">{Object.values(systemHealth.checks || {}).map((check) => <article key={check.label}><i aria-hidden="true">{check.status === 'operational' ? <CheckCircle size={21}/> : <WarningCircle size={21}/>}</i><div><strong>{check.label}</strong><span>{humanize(check.status)}</span></div></article>)}</div><div className="health-activity"><header><div><h3>Last 24 hours</h3><p>Operational signals from the application.</p></div></header><dl><div><dt>Failed emails</dt><dd>{systemHealth.activity?.failedEmails || 0}</dd></div><div><dt>Inbound webhooks</dt><dd>{systemHealth.activity?.inboundWebhooks || 0}</dd></div></dl></div>{(systemHealth.incidents || []).length > 0 && <div className="health-incidents"><header><div><h3>Open incidents</h3><p>Recent issues that may need attention.</p></div><span>{systemHealth.incidents.length}</span></header><div>{systemHealth.incidents.map((incident) => <article key={incident.id}><i data-severity={incident.severity} aria-hidden="true"><WarningCircle size={18}/></i><div><strong>{incident.source}</strong><p>{incident.message}</p><time>{date(incident.created_at)}</time></div></article>)}</div></div>}{systemHealth.error && <p className="admin-error" role="alert">{systemHealth.error}</p>}</> : <div className="health-loading"><Database size={24}/><strong>Checking services…</strong><span>Connecting to the health monitor.</span></div>}</section>}
      ...
      {adminView === 'messages' && <section className="admin-mailbox" aria-label="Customer messages"><aside className="mailbox-folders"><h2>Mailbox</h2><button type="button" className={messageFolder === 'all' ? 'active' : ''} onClick={() => setMessageFolder('all')}><Inbox size={19}/><span>Conversations</span><small>{messageThreads.filter((thread) => Boolean(thread.order_id)).length}</small></button><button type="button" className={messageFolder === 'support' ? 'active' : ''} onClick={() => setMessageFolder('support')}><ChatCircleText size={19}/><span>Support</span><small>{messageThreads.filter((thread) => !thread.order_id).length}</small></button><button type="button" className={messageFolder === 'sent' ? 'active' : ''} onClick={() => setMessageFolder('sent')}><PaperPlaneTilt size={19}/><span>Sent</span><small>{messageThreads.filter((thread) => thread.messages?.some((message) => message.direction === 'outbound')).length}</small></button><div><b>Order support</b><p>Every conversation remains attached to its customer order.</p></div></aside><section className="mailbox-list"><header><div><h2>{messageFolder === 'sent' ? 'Sent messages' : messageFolder === 'support' ? 'Support' : 'Conversations'}</h2><p>{visibleThreads.length} {visibleThreads.length === 1 ? 'thread' : 'threads'}</p></div><button type="button" onClick={() => startMessage()}><Plus size={17}/>New message</button></header><label><MagnifyingGlass size={17}/><input value={messageQuery} onChange={(event) => setMessageQuery(event.target.value)} placeholder="Search customer or order…" name="messageSearch" autoComplete="off" aria-label="Search customer or order"/></label><div className="mailbox-thread-list">{visibleThreads.length ? visibleThreads.map((thread) => { const lastMessage = thread.messages?.at(-1); return <button type="button" className={selectedThread?.id === thread.id ? 'active' : ''} onClick={() => setSelectedThreadId(thread.id)} key={thread.id}><i>{thread.customer_name?.charAt(0).toUpperCase()}</i><span><b>{thread.customer_name}</b><strong>{lastMessage?.subject || thread.subject}</strong><small>{lastMessage?.body_text || 'No messages yet'}</small></span><time>{lastMessage ? new Intl.DateTimeFormat('en-PH', { month:'short', day:'numeric' }).format(new Date(lastMessage.created_at)) : ''}</time></button> }) : <div className="mailbox-empty"><ChatCircleText size={26}/><b>No conversations yet</b><p>Send a message from the dashboard to start one.</p></div>}</div></section><article className="mailbox-reader">{selectedThread ? <><header><div><i>{selectedThread.customer_name?.charAt(0).toUpperCase()}</i><span><b>{selectedThread.customer_name}</b><small>{selectedThread.customer_email}</small></span></div><span className="mailbox-reader-status"><ChatCircleText size={15} aria-hidden="true"/>Active conversation</span></header><div className="mailbox-thread-head"><span>{selectedThread.orders?.order_number}</span><h2>{selectedThread.subject}</h2></div><div className="mailbox-messages">{selectedThread.messages?.map((message) => <section className={message.direction} key={message.id}><header><b>{message.direction === 'outbound' ? 'Tappy support' : selectedThread.customer_name}</b><time>{date(message.created_at)}</time></header><p>{message.body_text}</p><small data-status={message.delivery_status}>{message.delivery_status}</small></section>)}</div></> : <div className="mailbox-reader-empty"><EnvelopeSimple size={30}/><h2>Select a conversation</h2><p>Customer email history will appear here.</p></div>}{selectedThread && <form className={`mailbox-compose ${selectedThread?.order_id ? 'order-compose' : 'support-compose'}`} onSubmit={sendCustomerMessage} data-recipient={selectedThread?.customer_email || ''}><header><b>Reply to customer</b><span>Sent from Tappy support</span></header><label>Customer order<select required={Boolean(messageDraft.orderId)} value={messageDraft.orderId} onChange={(event) => { const order = orders.find((item) => item.id === event.target.value); setMessageDraft((current) => ({ ...current, orderId:event.target.value, subject:current.subject || (order ? `Regarding your Tappy order ${order.order_number}` : '') })) }}><option value="">Choose an order</option>{orders.map((order) => <option value={order.id} key={order.id}>{order.order_number} · {order.customer_name}</option>)}</select></label><label>Subject<input required name="subject" autoComplete="off" maxLength="180" value={messageDraft.subject} onChange={(event) => setMessageDraft({ ...messageDraft, subject:event.target.value })}/></label><label>Message<textarea required name="message" maxLength="10000" value={messageDraft.body} onChange={(event) => setMessageDraft({ ...messageDraft, body:event.target.value })} placeholder="Write a message…"/></label><footer><span>{messageDraft.body.length.toLocaleString()} / 10,000</span><button className="button" type="submit" disabled={messageSending}><PaperPlaneTilt size={17} aria-hidden="true"/>{messageSending ? 'Sending...' : 'Send message'}</button></footer></form>}</article></section>}
