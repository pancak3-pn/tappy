  ...
  return <main className="admin-page">
    ...
    <div className="admin-shell">
      ...
      {adminView === 'pages' && <section className="admin-pages-workspace">
        ...
        <form className="admin-page-editor" onSubmit={savePage}><header><div><span>{selectedPage ? 'Edit managed page' : 'Create managed page'}</span><h2>{pageForm.displayName || 'Untitled page'}</h2></div>{selectedPage && <div><button className="admin-copy-page" type="button" onClick={() => copyPageLink(selectedPage)}><Copy size={17}/>Copy profile link</button><a href={`/p/${selectedPage.public_id}`} target="_blank" rel="noreferrer" title="Open public page" aria-label="Open public profile"><Eye size={17}/></a></div>}</header>{selectedPage && <div className="admin-profile-url"><span>Customer profile URL</span><code>{`${window.location.origin}/p/${selectedPage.public_id}`}</code><button type="button" onClick={() => copyPageLink(selectedPage)}>Copy</button></div>}<div className="admin-page-editor-grid"><div className="admin-page-fields">
