...
export default function AdminDashboard() {
  ...
  return <main className="admin-page">
    ...
    <div className="admin-shell">
      <div className={`admin-title ${adminView === 'orders' ? 'admin-orders-title' : ''}`}><div><h1>{adminView === 'overview' ? 'Overview' : adminView === 'reports' ? 'Reports' : adminView === 'pages' ? 'Tappy Pages' : adminView === 'messages' ? 'Messages' : 'Orders'}</h1>{adminView === 'overview' && <p>{new Intl.DateTimeFormat('en-PH', { weekday:'long', month:'long', day:'numeric' }).format(new Date())}</p>}{adminView === 'orders' && <p>Manage and track all customer orders in one place.</p>}{adminView === 'messages' && <p>Customer conversations linked to their orders.</p>}{adminView === 'pages' && <p>{pages.length} managed pages</p>}</div>{adminView === 'orders' && <div className="admin-orders-total" aria-label={`${orderCounts.all} total orders`}><i aria-hidden="true"><ShoppingBag size={24}/></i><span><b>Total orders</b><strong>{orderCounts.all}</strong><small>Across all statuses</small></span></div>}</div>
