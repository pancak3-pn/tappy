  ...
  if (request.method === 'POST' && url.pathname === '/api/feedback/request') {
    ...
    try {
      ...
      const { data:activeToken, error:activeTokenError } = await supabase.from('feedback_tokens')
        .select('id').eq('order_id', order.id).is('used_at', null).gt('expires_at', new Date().toISOString()).order('expires_at', { ascending:false }).limit(1).maybeSingle()
      if (activeTokenError) return send(response, 503, { error:'Feedback is temporarily unavailable. Please try again.' })
      if (activeToken) return send(response, 409, { error:'A feedback link has already been sent for this order. Check your email or wait for it to expire.', alreadyRequested:true })
      const rawToken = randomBytes(32).toString('base64url')
      const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()
